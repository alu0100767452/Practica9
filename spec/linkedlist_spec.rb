...
=begin        @l_inv = List.new(@nodo5)
   	@l_inv.insert_head(@nodo4)
   	@l_inv.insert_head(@nodo3)
   	@l_inv.insert_head(@nodo2)
   	@l_inv.insert_head(@nodo1)
=end
	@l_inv=Examen.new(l.invert)
	expect(@ex2.exam).to eq(@l_inv)
